...
public class EventConsumerAdapter<T> {
        ...
        try {
            ...
            if (retryPolicy.shouldRetry(attempt, ex)) {
                throw new RetryableKafkaProcessingException(
                        ...
                        ex,
                        retryPolicy.backoffForAttempt(attempt)
                );
            }
    ...
    private void validateMetadataHeaders(Headers headers) {
        ...
        if (eventType == null || eventType.isBlank()) {
            throw new InvalidEventMetadataException("Missing required header: " + EventHeaders.X_EVENT_TYPE);
        }

        String versionRaw = headerValue(headers, EventHeaders.X_EVENT_VERSION);
        if (versionRaw == null || versionRaw.isBlank()) {
            throw new InvalidEventMetadataException("Missing required header: " + EventHeaders.X_EVENT_VERSION);
        }
        try {
        ...
        if (correlationId == null || correlationId.isBlank()) {
            throw new InvalidEventMetadataException("Missing required header: " + EventHeaders.X_CORRELATION_ID);
        }
    }
    ...
    private void validateEnvelopeIdentity(EventEnvelope<T> envelope) {
        if (!expectedEventType.equals(envelope.eventType())) {
            throw new InvalidEventMetadataException(
                    "Unexpected event type: expected=" + expectedEventType + " actual=" + envelope.eventType()
            );
        }
        if (expectedEventVersion != envelope.eventVersion()) {
            throw new InvalidEventMetadataException(
                    "Unexpected event version: expected=" + expectedEventVersion + " actual=" + envelope.eventVersion()
            );
        }
    }
