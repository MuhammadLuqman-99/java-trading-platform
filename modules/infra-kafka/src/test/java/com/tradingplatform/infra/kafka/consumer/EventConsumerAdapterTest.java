...
class EventConsumerAdapterTest {
    private final EventEnvelopeJsonCodec codec = new EventEnvelopeJsonCodec(EventObjectMapperFactory.create());
    ...
    void shouldRouteValidMessageToHandler() throws Exception {
        ...
        EventConsumerAdapter<OrderSubmittedV1> adapter = new EventConsumerAdapter<>(
    ...
    void shouldDeadLetterWhenMetadataIsMissing() throws Exception {
        ...
        verify(deadLetterPublisher).publish(eq(TopicNames.ORDERS_SUBMITTED_V1), eq(record), any(InvalidEventMetadataException.class));
    }
    ...
    void shouldDeadLetterWhenHandlerFailsAndNoRetryConfigured() throws Exception {
        ...
        verify(deadLetterPublisher).publish(eq(TopicNames.ORDERS_SUBMITTED_V1), eq(record), any(IllegalStateException.class));
    }

    private ConsumerRecord<String, String> createRecord(boolean includeVersionHeader) {
        EventEnvelope<OrderSubmittedV1> envelope = EventEnvelope.of(
                EventTypes.ORDER_SUBMITTED,
                1,
                "worker-test",
                "ord-1001",
        ...
        headers.add(EventHeaders.X_EVENT_TYPE, EventTypes.ORDER_SUBMITTED.getBytes(StandardCharsets.UTF_8));
        headers.add(EventHeaders.X_CORRELATION_ID, "ord-1001".getBytes(StandardCharsets.UTF_8));
        headers.add(EventHeaders.CONTENT_TYPE, EventHeaders.APPLICATION_JSON.getBytes(StandardCharsets.UTF_8));
